...
class Kilter(QWidget):
    def __init__(self):
        ...
        self.AD = ACCEPTDELETE()
        self.AD.closeSignal.connect(self.change)
    ...
    def drawStuff(self, box):
        for i in self.buttons:
            for j in i:
                if j:
                    j.deleteLater()
        if self.currentBox == None and self.windowTitle() == "мусорка":
            self.isInTrash = True
        elif len(self.upBox) >= 2 and self.upBox[1] == [None, "мусорка"]:
            self.isInTrash = True
        else:
            self.isInTrash = False
        print(self.isInTrash)
    ...
    def clickBox(self):
        ...
        if self.deleteRadioButton.isChecked():
            self.deleteStuff = ACCEPTDELETE(self.idTable[hi][wi][0], self.idTable[hi][wi][1], self.isInTrash)
...
class Kilter(QWidget):
    def __init__(self):
        ...
        self.sig = ACCEPTDELETE()
        self.sig.closeSignal.connect(self.change)
